...
NEURON {
    ...
    GLOBAL hslow,hfast :seed [GD removed seed from GLOBAL]
    RANGE mu_zn,mu_zm,mu_zkm,mu_zt,mu_zl
	:RANGE zl[4]
	GLOBAL vshift

	GLOBAL taukm : NF [GD removed NF from GLOBAL]
    RANGE seed, NF
    THREADSAFE
} : end NEURON
...
PARAMETER {
    gnabar  = 0.12   (S/cm2)
    gkbar   = 0.036  (S/cm2) :conductance density of usual voltage gated K 
    glbar      = 0.0003 (S/cm2)  
    gtbar      = 0.0003 (S/cm2)  
	gkmbar = .002   	(S/cm2) :conductance density of M-type K channels (muscaranic deactivated leak)
    gleak=0.00001		(S/cm2)
	eleak=-60 			(mV)
	NFleak=1
